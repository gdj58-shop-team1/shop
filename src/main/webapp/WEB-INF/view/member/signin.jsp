...
<script>
	$(document).ready(function() { // 이벤트 : <body>~</body>태그가 로드되고 나면 
		...
		$('#signinBtn').click(function() {
			...
			// customerPw2 미입력시
			if($('#customerPwCheck').val().length < 1) {
				
				alert('비밀번호를 입력해주세요');
				
				$('#customerPwCheck').focus();
				
				return false;
			}
			
			let customerPw = $('#customerPw').val();
			let customerPwCheck = $('#customerPwCheck').val();
			
			if(customerPw == customerPwCheck) {
				$('#customerSigninForm').submit();
			} else {
        ...
        <div class="flex flex-col overflow-y-auto md:flex-row">
          <div class="h-32 md:h-auto md:w-1/2">
            <img
              ...
              src="./loginCss/assets/img/create-account-office.jpeg"
              alt="Office"
            />
          </div>
          <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
            <div class="w-full">
            <form action="${pageContext.request.contextPath}/Signin" method="post" id = "customerSigninForm">
              <h1
              ...
              <label class="block mt-4 text-sm">
                ...
                <input
                  ...
                  placeholder="password"
                  type="password"
                  name = "customerPwCheck"
                  id="customerPwCheck"
                />
              </label>
           
              <button type = "button" id="signinBtn" class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
              		회원가입
              	</button>
